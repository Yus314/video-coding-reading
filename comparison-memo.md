...
P08・P10・P12は本文の実験節まで確認。P07の符号化順序/評価の注意、P18の構造/制御節、P20の分類/実験条件も確認済み。全件精読とは区別する。**著者報告の抽出であり、独立追試はしていない。** 未確認を未記載と読み替えない。
...
PDFから自動抽出されたグラフの数値表は原図との照合が未実施のため、この資料には採用していない。本文に明記された条件と限定的な解釈だけを使う。
...
現段階の `comparison-conditions.json` は論文単位の監査メモ。異なる精度・GPU・色空間の結果を一行へ統合した性能ランキングではない。実験単位データベースへの全面移行やコード再現は未実施。

## 条件が違うと分かった後、何なら言えるか

[後続研究の比較判断](followup-review.md)に、構造として比較できることと、数値の優劣を判断できない理由を分けた。DVC/DCVCの情報経路、状態伝播とfeature refresh、条件付き経路のbottleneckは読書上の対比に使える。

性能判断に進む時は、共通の実装版・モデル、時間構造、frame範囲、色形式、指標/集約、bit計測範囲を確認する。構造の違いを理解できたことは、同条件の性能実験を済ませたことではない。
