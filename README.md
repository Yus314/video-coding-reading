...
[目的別の到達点と終了条件](reading-queue.md)から、**原理理解／実装・運用／性能評価／研究テーマ探索**の一つを選んでください。必要な節だけを読み、目的を満たせば止められます。全候補の読了を前提にしません。

- **実装・運用**：設定なら[実装文書IC06](independent-review.md)、ニューラル方式の計時なら[測定境界](evidence-audit.md)から。読書と対象環境での動作確認は別です。
- **研究テーマ探索**：[分野地図](field-map.md)で問いを選び、既存解・別解・前提が破れる条件を比べます。資料庫の空白を未研究領域とは扱いません。
